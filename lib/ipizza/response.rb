class Ipizza::Response
  ...
  def verify(certificate_path, charset = 'UTF-8', count_characters_instead_of_bytes = false)
    param_order = @@response_param_order[@params['VK_SERVICE']]
    verify_params = param_order.inject(Hash.new) { |h, p| h[p] = @params[p]; h }
    mac_string = Ipizza::Util.mac_data_string(verify_params, param_order, count_characters_instead_of_bytes)
